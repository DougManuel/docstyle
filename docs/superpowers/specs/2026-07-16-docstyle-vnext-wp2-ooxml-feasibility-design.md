...
Package structure is fixed when the handle opens. `replace_part` accepts an existing non-metadata part only. `[Content_Types].xml` is package metadata rather than an addressable OPC part, and relationship parts are immutable after open. Task 8 reopens a completed output package through `opc.open_path` before publication, so the same content-type and relationship checks apply to the bytes that would be published.
