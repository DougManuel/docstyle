...
6. Archive path, duplicate-name, case-collision, symlink, encryption and limit tests fail closed before any unsafe decompression or allocation occurs, which is earlier than the publication boundary; the tests include cumulative-budget exhaustion across repeated reads under the candidate's documented cache-charging semantics, and central-directory/local-header name disagreement on entries the caller never requests.
