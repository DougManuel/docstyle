...
**Decision (atomic update procedure):** a writer produces new typed files under temporary names, writes a complete new manifest to a temporary name, then renames the manifest over `manifest.json` as the single commit point. Readers start from the manifest; a typed file whose hash does not match its manifest entry is stale and triggers regeneration for caches or a conflict for durable state. A failed update leaves the previous manifest and files intact.
